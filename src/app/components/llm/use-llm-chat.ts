...
import { PORTFOLIO_API_BASE } from "../../shared/seo/config";
...
export function useLlmChat() {
    ...
    async function hydrate() {
      ...
      try {
        const res = await fetch(
          `${PORTFOLIO_API_BASE}/api/llm/history?session_id=${encodeURIComponent(existing)}`,
  ...
  const fetchSuggestions = useCallback(async (history: ChatMessage[]) => {
    ...
    try {
      const res = await fetch(`${PORTFOLIO_API_BASE}/api/llm/suggestions`, {
        method: "POST",
  ...
  const send = useCallback(
    async (content: string) => {
      ...
      try {
        const res = await fetch(`${PORTFOLIO_API_BASE}/api/llm`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
